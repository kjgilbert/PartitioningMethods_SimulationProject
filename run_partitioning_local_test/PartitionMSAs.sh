...
#________go through each species file for a given gene sample (2 species each)________#
	...
	for f in files;
		...
		# make the partition file based on the gene sites stored in config_genes__.txt
			...
			echo "$aic_string" "$gene_string" "$rcluster_string" > 'partition_finder_'$dir'_raxml_aic.cfg'	
			echo "$aic_string" "$gene_string" "$greedy_string" > 'partition_finder_'$dir'_greedy_aic.cfg'	
			echo "$bic_string" "$gene_string" "$rcluster_string" > 'partition_finder_'$dir'_raxml_bic.cfg'
			echo "$bic_string" "$gene_string" "$greedy_string" > 'partition_finder_'$dir'_greedy_bic.cfg'	
			echo "$aicc_string" "$gene_string" "$rcluster_string" > 'partition_finder_'$dir'_raxml_aicc.cfg'
			echo "$aicc_string" "$gene_string" "$greedy_string" > 'partition_finder_'$dir'_greedy_aicc.cfg'	
...
done	 # all gene sample size directories
