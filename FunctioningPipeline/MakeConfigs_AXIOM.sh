...
for f in $files;
	...
	echo "$slurm_string_1"$raw_samp"_"$species'_noPart'"$slurm_string_2" > ${curr_path}'part_N'$raw_samp'_'$species'_noPart/Run.slurm'
...
echo \$(grep -A 2 newick ${curr_path}part_N${raw_samp}_${species}_r-a/best_scheme_nexus.txt.iqtree | tail -n 1) >> ${curr_path}part_N${raw_samp}_${species}_r-a/partitioning_treebuilding_output.txt" >> ${curr_path}'part_N'$raw_samp'_'$species'_r-a/Run.slurm'
	...
	# raxml BIC
	echo "python ${run_path}partitionfinder-2.1.1/PartitionFinderProtein.py ${curr_path}part_N${raw_samp}_${species}_r-b/ --raxml -p 4
conda deactivate
# copy nexus best scheme to a partition file
...
echo \$(grep -A 2 newick ${curr_path}part_N${raw_samp}_${species}_g-c/best_scheme_nexus.txt.iqtree | tail -n 1) >> ${curr_path}part_N${raw_samp}_${species}_g-c/partitioning_treebuilding_output.txt" >> ${curr_path}'part_N'$raw_samp'_'$species'_g-c/Run.slurm'
	...
	echo "
conda deactivate
${run_path}iqtree-1.6.12-Linux/bin/iqtree -s ${curr_path}part_N${raw_samp}_${species}_noPart/temp_align.phy -seed 123456789 -nt 4 -mrate G -bb 1000
echo \$(grep -A 12 MAXIMUM ${curr_path}part_N${raw_samp}_${species}_noPart/best_scheme_nexus.txt.iqtree) >> ${curr_path}part_N${raw_samp}_${species}_noPart/partitioning_treebuilding_output.txt
echo \$(grep -A 2 newick ${curr_path}part_N${raw_samp}_${species}_noPart/best_scheme_nexus.txt.iqtree | head -n 3 | tail -n 1) >> ${curr_path}part_N${raw_samp}_${species}_noPart/partitioning_treebuilding_output.txt
echo \$(grep -A 5 CONSENSUS ${curr_path}part_N${raw_samp}_${species}_noPart/best_scheme_nexus.txt.iqtree) >> ${curr_path}part_N${raw_samp}_${species}_noPart/partitioning_treebuilding_output.txt
echo \$(grep -A 2 newick ${curr_path}part_N${raw_samp}_${species}_noPart/best_scheme_nexus.txt.iqtree | tail -n 1) >> ${curr_path}part_N${raw_samp}_${species}_noPart/partitioning_treebuilding_output.txt" >> ${curr_path}'part_N'$raw_samp'_'$species'_noPart/Run.slurm'
